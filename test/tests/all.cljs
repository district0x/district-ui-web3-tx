  ...
  (run-test-async
    (let [instance (subscribe [::contracts-subs/instance :mintable-token])
      ...
      (wait-for [::accounts-events/accounts-changed]
        (dispatch [::deploy-events/deploy-contract :mintable-token {:gas gas-limit
                                                                    :from (first @accounts)}])
        ...
        (wait-for [::contracts-events/set-contract ::deploy-events/contract-deploy-failed]
          (dispatch [::events/send-tx {:instance @instance
                                       ...
                                       :tx-opts {:from (first @accounts) :gas gas-limit}
                                       :on-tx-hash [::tx-hash]
      ...
      (wait-for [::accounts-events/accounts-changed]
        ...
        (wait-for [::contracts-events/set-contract ::deploy-events/contract-deploy-failed]
          ...
          (dispatch [::events/send-tx {:instance @instance
                                       ...
                                       :tx-opts {:from (second @accounts) :gas gas-limit}
                                       :on-tx-hash [::tx-hash]
